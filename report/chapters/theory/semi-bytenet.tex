...
Using these ``monolingual'' translation model the loss function can extended:
\begin{equation}
\begin{aligned}
...
Beyond these issues there is also a big challenge in implementing BeamSearch using tensor operations as BeamSearch is highly state and sequence dependent. These details will not be discussed as it is implementation specific.

\begin{algorithm}[H]
  \caption{BeamSearch algorithm, specialized for NTM.}
  \begin{algorithmic}[1]
    \Function{BeamSearch}{$P(\mathbf{y}|\mathbf{x}), \mathbf{x}, state_0, b$}
      ...
      \Let{$i$}{$1$}
      \Repeat
        \Implicit{pack state ($\mathbf{s}_{i-1}$) so it looks like $(n \cdot b)$ observations.}
        \Let{$state_i, logits_i$}{\Call{Model}{$\mathbf{s}_{i-1}, \mathbf{x}$}} \Comment{2}
        \Implicit{unpack $state_i$ and $logits_i$ to it original shape of $n$ observations.}
        \State
...
Algorithm \ref{alg:theory:semi-supervised:nmt} shows a BeamSearch algorithm specialized for neural machine translation. The challenges listed earlier are solved in the following way:
