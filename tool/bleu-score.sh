...
# tokenize files
tokenizer -l $REF_LANG -no-escape < $REF_FILE > $WORK_DIR/tokenized.target.txt 2> /dev/null
tokenizer -l $REF_LANG -no-escape < $TRANSLATE_FILE > $WORK_DIR/tokenized.translate.txt 2> /dev/null
